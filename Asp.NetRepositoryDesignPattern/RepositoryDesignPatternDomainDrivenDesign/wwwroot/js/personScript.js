...
class PersonTableCl {
    ...
    updateGrid(abstractId, firstName, lastName, rowIndex) {
...
class PersonCrudCl {
    constructor() {
        this.table = document.getElementById('table');
        this.modal = new ModalCl();
        this.PersonTableCl = new PersonTableCl();
        this.table.addEventListener('click', this.handleEditClick.bind(this));
    ...
    handleTableClick(event) {
        if (event.target && event.target.classList.contains('bi-pencil')) {
            ...
            this.handleEditClick(event, rowIndex);
        }
    }
    handleEditClick(event, rowIndex) {
        ...
        if (row) {
            ...
            modal.setSaveCallback((newFirstName, newLastName) => {
                this.updateDatabase(abstractId, newFirstName, newLastName, row);
            });
        }
    }
    ...
    updateDatabase(abstractId, newFirstName, newLastName, row) {
        $.ajax({
            url: `http://localhost:5214/Person/Edit/${abstractId}`,
            type: 'POST',
            contentType: 'application/json',
            ...
            success: function (response) {
                ...
                this.PersonTableCl.updateGrid(response.abstractId, response.firstName, response.lastName, row);
    ...
    $.ajax({
        ...
        success: function (response) {
            response.forEach(function (person) {
                this.PersonTableCl.updateGrid(person.abstractId, person.firstName, person.lastName);
            });
...
//            console.log(abstractId);
//            this.modal.Open();
//            this.modal.setFieldValues(currentFirstName, currentLastName);
//            this.modal.setMessage("Edit the fields you want to update.");

//            this.modal.setSaveCallback((newFirstName, newLastName) => {
//                this.updateDatabase(abstractId, newFirstName, newLastName, row);
//            });
//        }
//    }
...
//    updateDatabase(abstractId, newFirstName, newLastName, row) {
//        fetch(`http://localhost:5214/Person/Edit/${abstractId}`, {
//            method: 'POST',
//            headers: {
//                'Content-Type': 'application/json'
//            },
//            body: JSON.stringify({ AbstractId: abstractId, FirstName: newFirstName, LastName: newLastName, Row: row })
//        })
//            .then(response => response.json())
//            .then(data => {
//                this.PersonTableCl.updateGrid(data.abstractId, data.firstName, data.lastName, row);
//                displayNotification("Data Edited successfully", "success"); // Show success notification
//                console.log('Data updated successfully');
//                this.modal.CloseModal();
//            })
...
//    loadInitialData() {
//        fetch(`http://localhost:5271/Person/GetPersonByIdJson`)
//            .then(response => response.json())
//            .then(data => {
//                data.forEach(person => {
//                    this.PersonTableCl.updateGrid(person.abstractId, person.firstName, person.lastName);
...
//    setFieldValues(firstName, lastName) {
...
//            .then(response => response.json())
//            .then(data => {
//                this.saveCallback(data.firstName, data.lastName);
//                displayNotification("Data Edited successfully", "success"); // Show success notification
//                console.log('Data updated successfully');
//                this.close();
//            })
...
//const personCrud = new PersonCrudCl();

//document.getElementById('table').addEventListener('click', function (event) {
//    if (event.target && event.target.classList.contains('bi-pencil')) {
//        const rowIndex = event.target.closest('tr').rowIndex;
//        personCrud.handleEditClick(event, rowIndex);
//    }
//});

//document.addEventListener('DOMContentLoaded', function () {
//    personCrud.loadInitialData();
//});
//```
